...
self.addEventListener('install', e => {
  ...
  // with stale bytes the Pages CDN was still serving.
...
self.addEventListener('fetch', e => {
  ...
  if (e.request.mode === 'navigate') {
    ...
    // of always being one full load behind. Cache is the offline fallback.
    e.respondWith(
      fetch(e.request.url, { cache: 'no-cache' }).then(res => {
        if (res && res.ok) { const cl = res.clone(); caches.open(C).then(c => c.put(e.request, cl)); }
        return res;
      }).catch(() =>
        caches.match(e.request).then(hit => hit || caches.match('./Theory%20Trainer.dc.html'))
      )
    );
    return;
  }
