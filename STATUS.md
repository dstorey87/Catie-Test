...
- ✅ **Road signs** (v16, #44): official GOV.UK pictures (DfT traffic sign images; Highway Code markings and lights), 37 in one map in `signs.js` (`TTSigns.list`), files in `signs/`. Home → Road signs shows each with its official meaning; **Quiz me on the signs** asks bank questions that have a picture. Credited (Crown copyright, OGL v3.0) there, in About and in Help. To add a picture: file in `signs/`, an entry in the map, `./signs/<file>` in `sw.js` EXTRAS — `tests/signs.test.js` fails until all three agree. Live bank `questions.sign` set by `supabase/data/2026-09-25-sign-images.sql` (applied 2026-09-25). Every Highway Code sign by category, with a quiz and Adventure worlds: #48, in progress Progress is saved in her record under `adventure` (best score, stars, plays) and the app keeps it when it saves; answers and XP join her normal ones. It opens offline (in `sw.js` CORE). On the free sample it says "Sign in to the app to unlock every world". Test: `node --test tests/coach.test.js tests/adventure-page.test.js`; in a browser, see How to test
...
## How to test
- `node --test` runs every test (422 at v16); CI runs it on every pull request and every push to `develop` and `main`
