...
- ✅ **Adventure mode** (v13, issue #27): https://dstorey87.github.io/Catie-Test/adventure.html — a Duolingo-style route through the 14 topics, drawn as a winding road. Each topic is a world of 7-question lessons and a 10-question checkpoint (70 stages with today's bank). 80% right passes a stage and opens the next; stars at 80%, 90% and 100%; any passed stage can be replayed; a missed question comes back once; flags show in her Flagged screen. 22 worlds since v17: the 14 topics, then 8 sign worlds (one per Highway Code category) on their own road, open from the start. How to use it: choose the learner in the app, then Home → **Adventure** (just under Today's lesson) → tap the stop with the car on it → Start. Since v16 (#47) its answers count toward Home's goal ring and streak, and it follows Settings → text size, easy-reading font, high contrast, reduce motion, theme and read aloud (speakers beside the question, each option and the explanation).
- ✅ **Road signs** (v16 #44, v17 #48): Home → Road signs. Every sign, road marking, light signal and vehicle marking in The Highway Code — 205 official pictures with its caption word for word — under the pages' own headings, plus 7 DfT signs; category buttons jump to each. **Quiz me on all signs** / **Quiz me on this category**: official picture, its caption as the answer, three other captions from the same category as the wrong ones. Sign answers count toward goal, streak and XP; the bank's coach never sees them (ids `sign:<key>`). One map, `signs.js` (`TTSigns.list`, 212 entries); pictures in `signs/` (3.8 MB); `tests/data/highway-code-catalogue.json` holds the researched catalogue and every left-out picture with its reason; `tests/signs.test.js` checks every caption against it. Only the 30 pictures bank questions use are in `sw.js` EXTRAS; the rest are cached when first seen. Credited (Crown copyright, OGL v3.0) there, in About and in Help. Left out: the arm-signal pages (photographs of people, not decided). Live bank `questions.sign`: `supabase/data/2026-09-25-sign-images.sql` (applied 2026-09-25)
